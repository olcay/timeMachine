...
(function () {
	...
	(function () {
		...
		var $form = document.querySelectorAll('#time-console-form')[0],
			...
			$date = document.querySelectorAll('#time-console-form input[type="date"]')[0],
			...
			$xmlhttp.onreadystatechange = function () {
				if (this.readyState == 4) {
					if (this.status == 200) {
						...
						var beforeEvent, afterEvent, visorText;

						var i;
						for (i = 0; i < eventArray.length; i++) {
							if (new Date(eventArray[i].date) <= $date.valueAsDate) {
						...
						if (afterEvent && ($date.valueAsDate - new Date(beforeEvent.date) > new Date(afterEvent.date) - $date.valueAsDate)) {
